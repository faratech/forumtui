...
/// A video playing in a pane.
///
/// The frame itself is painted by the app (only `ratatui-image`'s widget may
/// write image escapes — hard rule 1), so this reserves the rect, draws the
/// caption and the transport line, and records where the picture goes.
pub fn render_video_view(
    s: &mut super::VideoViewState,
    f: &mut Frame,
    area: Rect,
    theme: &Theme,
    g: &Glyphs,
    ...
    // A failure inside the playback thread (resolution, a dead stream)
    // surfaces here rather than as a pane that silently never starts.
    if s.error.is_none()
        && let Some(p) = &s.playback
        && let Some(e) = p.error()
    {
        s.error = Some(e);
    }
    if let Some(err) = &s.error {
        f.render_widget(
            Paragraph::new(format!("Error: {err}\n\nPress o to watch it in a browser."))
                .style(Style::new().fg(theme.error)),
            inner,
        );
        return;
    ...
    if !s.images.inline() {
        f.render_widget(
            Paragraph::new(
                "This terminal has no inline graphics \u{2014} press o to watch it in a browser."
                    .to_string(),
            )
            .style(theme.dim()),
            inner,
    ...
    // One row of transport at the bottom; the rest is picture.
    let pic_rows = inner.height.saturating_sub(1);
    let state = match &s.playback {
        Some(p) if p.paused() => "paused",
        Some(p) if p.finished() && s.shown > 0 => "ended",
        Some(_) if s.shown == 0 => "buffering\u{2026}",
        Some(_) => "playing",
        None => "stopped",
    };
    f.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled(format!("{} ", g.play), theme.dim()),
            Span::styled(state.to_string(), theme.dim()),
            Span::styled(
                "   Space pause \u{b7} o browser \u{b7} Esc close".to_string(),
                theme.faint(),
            ),
        ])),
    ...
    if pic_rows > 0 {
        s.rect = Rect::new(inner.x, inner.y, inner.width, pic_rows);
    }
}

pub fn video_view_key(s: &mut super::VideoViewState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Char('q') | KeyCode::Char('h') | KeyCode::Left => Action::PopScreen,
        KeyCode::Char(' ') => {
            match s.playback.as_mut() {
                // Pause stops decode and sound at the same instant, so they
                // cannot drift apart while stopped.
                Some(p) => p.toggle_pause(),
                None => return Action::Notice("Nothing is playing.".into()),
            }
            Action::None
        }
        KeyCode::Char('o') => Action::OpenUrl(s.url.clone()),
        _ => Action::None,
    }
}

pub fn video_view_hints(s: &super::VideoViewState) -> Hints {
    let mut keys: Vec<(&str, &str)> = Vec::new();
    if let Some(p) = &s.playback {
        keys.push((" ", if p.paused() { "resume" } else { "pause" }));
    }
    keys.push(("o", "browser"));
    keys.push(("Esc", "close"));
    Hints::new(&keys, 0)
